...
public final class DataSeries implements Serializable, Iterable<DataPoint> {
	...
	public JsonObject toJsonOut() {
		...
		String minTimeJson = Long.toString(minTime);
		String intervalJson = Long.toString(interval);
		JsonArray dataArray = new JsonArray();
		for (double dataPointValue: data) {
			dataArray.add(dataPointValue);
		}
		JsonArray presentArray = new JsonArray();
		for (boolean presentPointValue: present) {
			presentArray.add(presentPointValue);
		}
		JsonObject dataPoint = new JsonObject();
		dataPoint.addProperty("minTime", minTimeJson);
		dataPoint.addProperty("interval", intervalJson);
		dataPoint.add("data", dataArray);
		dataPoint.add("present", presentArray);
		return dataPoint;
	...
	public double getMi() {
		...
		for (DataPoint dataPoint : dataPoints) { sum += dataPoint.getValue(); }
		return sum / dataPoints.size();
	}

	public Pair<DataPoint, DataPoint> getSmallestAndLargestDataPoint() {
		DataPoint smallest = new DataPoint(Long.MAX_VALUE, Long.MAX_VALUE);
		DataPoint largest = new DataPoint(Long.MIN_VALUE, Long.MIN_VALUE);
		for (DataPoint dataPoint : getDataPoints()) {
			if (smallest.getValue() >= dataPoint.getValue()) {
				smallest = dataPoint;
			}
			if (largest.getValue() <= dataPoint.getValue()) {
				largest = dataPoint;
			}
		}
	...
	public NumberAxis getScaledYAxis() {
		Pair<DataPoint, DataPoint> smallestAndLargest = this.getSmallestAndLargestDataPoint();
		DataPoint smallest = smallestAndLargest.getKey();
		DataPoint largest = smallestAndLargest.getValue();
		double mi = this.getMi();
		double paddedMaximum = 2 * largest.getValue() - mi;
		double paddedMinimum = smallest.getValue() - (largest.getValue() - mi);
		if (paddedMinimum < 0) paddedMinimum = 0;
		double steps = (paddedMaximum - paddedMinimum) / this.getDataPoints().size();
		if (steps <= 1.0) steps /= 10;
		return new NumberAxis(paddedMinimum, paddedMaximum, steps);
	}
}
